# Unified EDA Platform Building Roadmap
...
- **Submodules** map to **tasks / sub-issues** (can be done, in one sitting, each has a *Done when* test).
- Every submodule is written to be **built and tested incrementally** — there's always something runnable at the end of each one.
- Timelines are **rough, solo, part-time** estimates for planning only. Time estimate includes learning curve.

---

## Guiding principles (architecture at a glance)

1. **One design, many views.** A component (a *cell*) has several representations (*views*): a symbol, a SPICE model, a footprint, a physical layout. Every tool in the platform is an engine that reads/writes the view it cares about. More focus on foundational abstraction for a smoother ride later.
2. **Shared backbone.** The connectivity graph (cells, instances, pins, nets) is common to simulation, PCB, and IC. Focus on this first then per-view specialization comes later.
3. **Prototype in Python, ship in Rust.** Prove algorithms fast in Python/NumPy with a test suite; port the stabilized modules to Rust for the product, validated against the Python version as an oracle (bridge via PyO3/maturin perhaps, or other, to avoid heavy migration).
4. **Vertical slices over invisible plumbing.** Building one trivial thing to work end-to-end (schematic → simulate → export) before deepening any single layer.
...
3. **Differential (oracle)** — run two implementations that *must* agree and diff them: incremental vs brute-force, Python prototype vs Rust port, circuitor engine vs a reference tool.
...
**Oracles to lean on:** hand-derived analytic answers (circuit math); brute-force recomputation (net index); the Python prototype (for the Rust port); established tools — ngspice, Icarus Verilog / Verilator, KiCad / gerbv, KLayout / Magic / Netgen.
...
**Overview.** Set up the repo, tooling, CI, and the project board itself. Locking the language/GUI strategy so there's no big changes later.
...
**Testing.** The pipeline itself is the deliverable, so test the harness: a trivial smoke test (builds + imports) must pass in CI (0.3); a deliberately broken commit (lint error or failing assert) must be **rejected** — verify the gate actually blocks it; confirm pre-commit hooks fire locally (0.2). *Continuous check:* the green CI badge becomes the glance health signal from here on.
...
- [ ] **1.1 Core object model** — [ ] Cell type + view registry · [ ] Instance referencing a cell · [ ] Pin/terminal on cells · [ ] Net (hyperedge over pins) · [ ] Hierarchy (cell contains instances). *Done when:* construct the voltage divider (V1, R1, R2, nets vin/out/gnd) in code and query "all pins on net `out`".
...
**Tools & why.** Prototype with **PySide6 (Qt) + `QGraphicsView`** — it has pan/zoom/selection/undo scaffolding purpose-built for exactly this. For the Rust product, choose **egui** (immediate-mode, lightweight) or **Tauri + WebGPU** (web-tech frontend). R-tree for hit-testing. Study Logisim-evolution and hneemann's *Digital* (both readable Java editors).
...
- *Hand-calculated fixtures (3.1, 3.4, 3.5):* voltage divider (Ohm's law), RC step V(t)=V₀(1−e^(−t/RC)), RLC damping/resonance, low-pass −3 dB corner and −20 dB/decade slope are derive by hand and assert within tolerance. This is the "solve it by hand" layer.
- *Per-stamp unit tests (3.1):* assert a resistor/source stamp writes the exact matrix entries.
- *Regression vs ngspice (3.3–3.6):* run the identical netlist in ngspice and diff node voltages/waveforms within tolerance — primary oracle for nonlinear and device work.
- *Integrator order test (3.4):* integrate dy/dt=−y against e^(−t); halve Δt and confirm the error shrinks at the method's expected order (validates the numerical integration, not only the answer).
- *Conservation (3.4):* a lossless LC tank must keep total energy bounded over long runs.

**Timeline:** 10–20 weeks (3.1–3.4 are the core; 3.6 is endless, I need to stop wherever I deem sufficient).

---

## Phase 4 — Digital simulator

**Overview.** An event-driven simulator over the same netlist. Numerically trivial vs SPICE (the designed circuitor engine).
...
**Tools & why.** Write the event engine. Study **Logisim-evolution** / **Digital** for structure. Add HDL text import later via **`sv-parser`** (Rust) or **ANTLR** grammars and Verilator/Yosys for synthesis, if needed.
...
- *Co-simulation cross-check (4.2–4.4):* feed identical stimulus to circuitor engine and to Icarus Verilog / Verilator, then compare the VCD traces.
...
**Timeline:** 4–8 weeks (not considering HDL).
...
**Tools & why.** **Clipper2** for polygon boolean/offset (exactly what clearance/isolation needs) + **R-tree** for DRC queries. Do **interactive routing first**; delegate autorouting to **FreeRouting** or defer it. **Write the Gerber/Excellon exporter by hand** from the Ucamco spec — gives full control; the exporter is the bridge to fab, mill (via FlatCAM/pcb2gcode → G-code → GRBL), or ink printer (Voltera/DragonFly). OpenCASCADE for STEP/3D later.
...
- [ ] **5.7 (Optional) CAM bridge** — [ ] emit G-code isolation toolpaths · [ ] stream to a GRBL machine. *Done when:* a mill cuts a real board from circuitor's output.
...
- *Manufacturing validation (5.6):* parse the Gerber/Excellon back with `gerbv`/`libgerbv`/`pcb-tools` and compare to the source; run a Gerber linter / fab preflight; export→parse→compare round-trip.
- *Visual regression:* snapshot-diff the rendered board.
- *Physical acceptance (5.7):* mill/print a board and test it with a multimeter, checking for shorts & opens (continuity).

**Timeline:** 12–24 weeks (5.4 routing UX and 5.6 export are the biggest chunks).

---

## Phase 6 — IC / custom layout (lifetime wing)

**Overview.** The Cadence-level grand finale. **Integrate** the mature open-silicon ecosystem rather than rebuilding it; treat GDSII/OASIS like another view circuitor EDA imports/exports.
...
- [ ] **6.1 GDSII/OASIS I/O** — [ ] import/export via gdstk as a DB view. *Done when:* a GDS round-trips through circuitor's custom database.
- [ ] **6.2 Layout viewer/editor integration** — [ ] embed/drive KLayout. *Done when:* a cell's layout view opens in the EDA.
...
**Testing.** Will use or attempt to emulate silicon tools here:

- *DRC/LVS as tests (6.3):* a correct layout must pass Magic DRC and Netgen LVS, an intentionally broken one must fail each.
- *GDSII round-trip (6.1):* export→import via `gdstk`→compare geometry.
- *Cross-check (6.2):* diff circuitor's DRC results against KLayout's on the same layout.
- *Reference cells:* regress Circuitor's flow output against known-good SKY130 standard cells.

**Timeline:** 6+ months, open-ended.

---

## Cross-cutting concerns (ongoing, every phase)

- **Interchange formats** — SPICE netlist, Verilog/VHDL, Gerber/Excellon, GDSII/OASIS, DEF/LEF, IPC-2581. Supporting them is how the platform interoperates *and* how to bootstrap by importing existing designs.
...
- Tagged each issue with an effort estimate and pulled from the phase timelines above.
